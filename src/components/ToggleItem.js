    ...
    <button className="w-full" onClick={toggleLayer(props.layerID)}>
      <span className="relative flex p-2 text-left text-base font-normal text-gray-900 rounded-lg hover:bg-gray-200">
        <span className="flex-auto ml-3 select-none">{props.layerID}</span>
        {
          notification
            ?<div>
              <svg className="absolute -top-0.5 -left-0.5 animate-ping w-3 h-3" fill={props.tint} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="50"/> </svg>
              <svg className="absolute top-0 left-0 w-2 h-2" fill={props.tint} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="50"/> </svg>
            </div>
            :<div></div>
        }
        {
          eyeShow
            ...
            :<svg className="h-6" fill={props.tint} viewBox="0 0 24 24"><path d="M15 12c0 1.654-1.346 3-3 3s-3-1.346-3-3 1.346-3 3-3 3 1.346 3 3zm9-.449s-4.252 8.449-11.985 8.449c-7.18 0-12.015-8.449-12.015-8.449s4.446-7.551 12.015-7.551c7.694 0 11.985 7.551 11.985 7.551zm-7 .449c0-2.757-2.243-5-5-5s-5 2.243-5 5 2.243 5 5 5 5-2.243 5-5z"/></svg>
        }
      </span>
    </button>
  );
